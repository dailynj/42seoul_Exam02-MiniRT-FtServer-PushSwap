...
t_bool hit_cylinder(t_object *world, t_ray *ray, t_hit_record *rec)
{
    ...
    if (cylinder_discriminant(cy, ray, rec) == FALSE)
        return (FALSE);
    rec->p = ray_at(ray, rec->t);
    rec->normal = vec_unit(vec_minus(vec_minus(rec->p, cy->point), vec_mul_num(
					vec_dot(cy->normal, vec_minus(rec->p, cy->point)), cy->normal)));
